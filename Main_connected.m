...
% Saving                          
FileName = fullfile(['C:\Users\admin\Documents\Willem\acdc_centralized_controller\results\',datestr(now,'mm-dd-yyyy HH-MM') ,'_',TYPE,'_connected.mat']);
emptyStruct = struct;
...
% disp(['OPF started at ', num2str(round(toc(T_outer),2)), ' seconds'])


for n_inner = 1:3

    if n_inner == 3 && toc(T_outer) > 0.8
        continue
    end
    ...
    [solution,OPF_termination] = make_optimizer_connected_V3_3(Grid_para,Filter_para,idx,constraints,temp_variables,prev_solution,OPF_termination,SM_info,Ts,state);
...
% disp(['OPF finished in ', num2str(round(toc(T_outer),2)), ' seconds'])
...
% solution.P_quad_ic*Grid_para.A_b;
...
if n_outer ~= 1 && LIMIT_sp == true
    ...
    solution.Sambat_P = setpoint_limit(solution.Sambat_P,prev_solution.Sambat_P,P_limit);
...
elseif solution.proceed == 0
    fprintf(2,'At least one setpoint is not valid -> Previous setpoint is implemented \n')
...
%AFE
setpoint_AFE = [solution.IC_E*Grid_para.Vdc_b; solution.IC_P*Grid_para.A_b; constraints.Edc_nominal*Grid_para.Vdc_b; -solution.IC_Q*Grid_para.A_b; 0]; 
%SUPERCAP
