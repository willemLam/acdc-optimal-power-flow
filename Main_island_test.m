...
% Saving                          
FileName = fullfile(['C:\Users\admin\Documents\Willem\acdc_centralized_controller\results\',datestr(now,'mm-dd-yyyy HH-MM') ,'_island_',TYPE,'.mat']);
emptyStruct = struct;
...
[SM_info,variables] = ReceiveFromStateMachine_island(Grid_para,variables);
% SM_info.Angle = SM_info.Angle - variables.Eang(idx.vscac_vv);
...
for n_inner = 1:5
    ...
    solution.Eabs(23:26) = [constraints.Edc_nominal,0,0,constraints.Edc_nominal]; %constraints.Edc_nominal; %90.9375;
    solution.Eabs(27) = constraints.Edc_nominal; %0.9375;%
    solution.Q(19:22) = [0,0.073,0.073,0.073];
    solution.P(27) = 0.0;
    solution.SupCap_P = 0.0;
    solution.Sambat_P = 0.0;
    solution.PV_P_perun = 0.0;
    solution.PV_P_solarmax = 0.0;
...
SM_info.Angle = 0;%SM_info.Angle*(180/pi);
SM_info.Magnitude = 230;%SM_info.Magnitude*(Grid_para.V_b/sqrt(3));
SM_info = sendToStateMachine_island(SM_info);
