function [Io, Iph, Rs, Rsh, n] = est_single_diode_param(I, V, NsVth)
% EST_SINGLE_DIODE_PARAM regression technique to fit the single diode 
...
%
% Description
%   est_single_diode_param uses a regression technique based on [1] to
...
if length(I)~=length(V)
    msgbox({'Error in est_single_diode_param'; 'current and voltage vectors are different length'})
    return
end
...
sCC=(CC(:)-nanmean(CC)); % center co-content values
