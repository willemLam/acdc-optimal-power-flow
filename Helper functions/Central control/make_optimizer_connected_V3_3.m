function [solution,OPF_termination] = make_optimizer_connected_V3_3(Grid_para,Filter_para,idx,constraints,variables,prev_solution,OPF_termination,SM_info,Ts,state)

%% initialize yalmip
optimal.P_pcc = sdpvar(1,1); %B1
optimal.Q_pcc = sdpvar(1,1); %B1
optimal.Plossdc = sdpvar(1,1);
...
% con = [con (optimal.Pdc_ic3 <= state.P(24)+Delta_P)&(optimal.Pdc_ic3 >= state.P(24)-Delta_P)]; %B22
...
con = [con (optimal.Pdc_ic1 == -sum(optimal.P([23:27])) + optimal.Plossdc)];
...
if     SM_info.mode == "Grid connected"
    cost.sum =    0*cost.P_afe1_norm + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm +   5*cost.P_afe_norm + 10*cost.PCC_q_norm + cost.P_samsung_norm; 
elseif SM_info.mode == "Prepare Island" % here
    cost.sum =   100*cost.P_afe1_norm + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm + 0.5*cost.P_afe23_norm + 10*cost.PCC_q_norm + 0.1*cost.P_samsung_norm +  10*cost.Q_afe1_norm  + cost.PCC_p_norm; 

%     cost.sum =   100*cost.P_afe1_norm + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm +  0.5*cost.P_afe23_norm + 10*cost.PCC_q_norm + 0.1*cost.P_samsung_norm + cost.PCC_p_norm +   10*cost.Q_afe1_norm; 
elseif SM_info.mode == "Forming but connected" % here
    cost.sum =   100*cost.P_afe1_norm + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm + 0.5*cost.P_afe23_norm + 10*cost.PCC_q_norm + 0.1*cost.P_samsung_norm + cost.PCC_p_norm +   10*cost.Q_afe1_norm; 
elseif SM_info.mode == "Island"
    cost.sum =    0*cost.P_afe1_norm + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm +   5*cost.P_afe_norm + 10*cost.PCC_q_norm + 0.1*cost.P_samsung_norm + cost.PCC_p_norm + 0*10*cost.Q_afe1_norm; 
