...
%     resolution = 300;
%     value = [];
    t0 = matlabToEpochTime(datenum(2023, 9, 4,12,0,0));
    tf = matlabToEpochTime(datenum(2023, 9, 4,13,0,0));
    seriesname = 'PVroof';
    tag_keys = { 'device' };
    tag_values = { 'PVroof' };
    resolution = 60*15;
        database= 'ELL rooftop PV'; %'logging';
...
header = '--header "Authorization: Token o5_xKQX10SAvf8xZ8DMnSKJv9nvKoZiJbMOTJFd1ceU3RMGNnn--ok_oZhIBWhp6RVdo-tOfu7WIyVQhw3zTlA=="'; %EwKjcJKOVEwAsEhaQqPiNx5-Y0OBOpNNrKG6z3nXslgx0cBue9S5ZT133z34HHKn92F6oX2l36upO-3Fl6KWxQ=="'; 

[exit_state, result] = system(sprintf('curl -X POST http://%s:8086/query?pretty=true -sS -H Accept:application/json > result_temp.json %s --data-urlencode db="%s" --data-urlencode q="%s"', IP, header, DB, query)) ;

% [a, b] = system(sprintf('curl -X POST "http://192.168.1.58:8086/query?pretty=true" -sS -H Accept:application/json > result.json --header "Authorization: Token X64jRFWeFhWFHHCkQvcDFPdmLVQX11l4aIamdObHNwnKFZW0YZQlTgTcBoswf1L5CeF-u0JJF5vzFmW2w_T7Mw==" --data-urlencode "db=logging" --data-urlencode "q=SELECT "GHI" FROM GITbox WHERE time > ''2021-02-01T10:19:59Z'' and time < ''2021-02-01T10:24:59Z'' AND "box"=''3''"'));
