function [solution,OPF_termination] = make_optimizer_connected_V2(Grid_para,idx,constraints,variables,prev_solution,OPF_termination)
...
optimal.SupCap_P = sdpvar(1,1); %B27
...
con = [con (optimal.PV_Q_perun <= 0.3)&(optimal.PV_Q_perun >= -0.3)]; %B7
...
con = [con (optimal.SupCap_P <= constraints.SupCap_Pmax)&(optimal.SupCap_P >= constraints.SupCap_Pmin)]; %B27
...
% optimal.P(27) = optimal.PV_P_emul7 + optimal.SupCap_P ;
% 
% optimal.P(1) = optimal.slack.P;                                 % GCP is a zero injection node
...
% optimal.P(19:26) = variables.P(19:26); 
optimal.P = [optimal.slack.P; 
             zeros(1,1);
             variables.P(3);
             zeros(1,1);
             variables.P(5);
             ...
             zeros(4,1);
             sdpvar(8,1);
             optimal.PV_P_emul7 + optimal.SupCap_P;
             optimal.PV_P_emul6;
             optimal.PV_P_emul5;
             optimal.PV_P_emul4
             ] ;
...
% optimal.Q([19,20,21,22]) = [optimal.IC1_Q,optimal.IC2_Q ,optimal.IC3_Q, optimal.IC4_Q]; % AFE in Vdc-Q mode
% 
% optimal.Q(1) = optimal.slack.Q;                                 % GCP is a zero injection node
...
optimal.Eabs(idx.vscdc_vq) = [optimal.IC1_E, optimal.IC2_E, optimal.IC3_E, optimal.IC4_E]; % AFE in Vdc-Q mode
optimal.Eabs(idx.vdc) = variables.Eabs(idx.vdc);
optimal.Eabs(idx.slack) = variables.Eabs(idx.slack);
...
% optimal.Eang(idx.slack) = variables.Eang(idx.slack);
% 
...
optimal.Iabs = variables.Iabs - K.Iabs.P*(variables.P - optimal.P)  ...
                              ...
                              - K.Iabs.Eang*(variables.Eang  - optimal.Eang); 
...
con = [con optimal.slack.P == -sum(optimal.P(setdiff(1:Grid_para.n_nodes,idx.slack))) + (optimal.Ploss)];
con = [con optimal.slack.Q == -sum(optimal.Q(setdiff(1:Grid_para.n_nodes,idx.slack))) + (optimal.Qloss)];
...
Edc_nominal = 750/800; % variables.E_slack_sp; %
cost.IC.E = 10* (norm([(optimal.IC1_E - Edc_nominal), (optimal.IC2_E - Edc_nominal), (optimal.IC3_E - Edc_nominal), (optimal.IC4_E - Edc_nominal)],2));
...
cost.supcap = norm([optimal.SupCap_P]);
...
cost.sum = cost.IC.E + cost.loss  + cost.slack.Q +  cost.PV_P + cost.IC.Q + 0*cost.PV_Q + cost.sambat +cost.supcap; 
% cost.sum = cost.IC.E + cost.loss +  cost.PV_P + cost.IC.Q + cost.sambat +cost.supcap; 
...
if sol.problem == 0
    ...
    solution.Sambat_P = double(optimal.Sambat_P);
    solution.SupCap_P = double(optimal.SupCap_P);
