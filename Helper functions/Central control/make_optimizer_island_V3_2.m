function [solution,OPF_termination] = make_optimizer_island_V3_2(step,SM_info,Grid_para,Filter_para,idx,constraints,variables,prev_solution,OPF_termination,Ts,state)


%% initialize yalmip
optimal.P_pcc = zeros(1,1); %B1
optimal.Q_pcc = zeros(1,1); %B1
optimal.Ptotdc = sdpvar(1,1);
...
con = [con (optimal.Pslack_ac == -sum(optimal.P([1:18,20:21])) + optimal.Plossac)];
con = [con (optimal.Pslack_ac <= state.P(19) + Delta_P)&(optimal.Pslack_ac >= state.P(19) - Delta_P)]; %B22
...
if     SM_info.mode == "Grid connected"
    cost.sum = 0*100*cost.I_afe_norm + 100*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe23_norm + 0.5*cost.P_afe23_norm + 0.1*cost.P_samsung_norm + 10*cost.Q_afe1_norm; 
