function [solution,OPF_termination] = make_optimizer_island_V3_3(step,SM_info,Grid_para,Filter_para,idx,constraints,variables,prev_solution,OPF_termination,Ts,state)


%% initialize yalmip
optimal.P_pcc = zeros(1,1); %B1
optimal.Q_pcc = zeros(1,1); %B1
optimal.Plossac = sdpvar(1,1);
optimal.Pslack_ac = sdpvar(1,1);
optimal.Pslack_dc = sdpvar(1,1);
...
% SC is 0.196875kwh
% 90% efficiency
con = [con (optimal.SOC_sc == SM_info.SC_soc/100 - (optimal.P_sc*Ts/3600)/(196.875/Grid_para.A_b))]; %sign: + is inject, - is charge
...
constraints.I_max2 = constraints.I_max;
if SM_info.mode == "Island" 
    constraints.I_max2(10) = constraints.I_max(10) + 0.6/Grid_para.I_b;
end

con = [con (optimal.Iabs <= constraints.I_max2)&(optimal.Iabs >= -constraints.I_max2)];
...
Delta_P = 500/Grid_para.A_b;
% con = [con (optimal.Pac_ic2 <= state.P(20)+Delta_P)&(optimal.Pac_ic2 >= state.P(20)-Delta_P)]; %B22
% con = [con (optimal.Pac_ic3 <= state.P(21)+Delta_P)&(optimal.Pac_ic3 >= state.P(21)-Delta_P)]; %B22

% optimal.Ptotdc = optimal.Pac_ic2 + optimal.Pac_ic2 + optimal.P_sc; %B22
% state.Ptotdc = -(state.P(23) + state.P(24) + state.P(25));

% variables.Plossdc = real(sum(variables.S(22:27)));
% con = [con (optimal.Plossdc == variables.Plossdc -  K.Rloss.P(22:27)*(variables.P(22:27) - optimal.P(22:27))...
%                                                  -  K.Rloss.Q(22:27)*(variables.Q(22:27) - optimal.Q(22:27))...
%                                                  -  K.Rloss.Eabs(22:27)*(variables.Eabs(22:27)  - optimal.Eabs(22:27))...
%                                                  -  0*K.Rloss.Eang(22:27)*(variables.Eang(22:27)  - optimal.Eang(22:27)))];

%%limit variation in AC slack
variables.Plossac = real(sum(variables.S(1:21)));
con = [con (optimal.Plossac == variables.Plossac -  K.Rloss.P(1:21)*(variables.P(1:21) - optimal.P(1:21))...
                                                 -  K.Rloss.Q(1:21)*(variables.Q(1:21) - optimal.Q(1:21))...
                                                 -  K.Rloss.Eabs(1:21)*(variables.Eabs(1:21)  - optimal.Eabs(1:21))...
                                                 -  0*K.Rloss.Eang(1:21)*(variables.Eang(1:21)  - optimal.Eang(1:21)))];

con = [con (optimal.Pac_ic1 == -sum(optimal.P([1:18,20:21])) + optimal.Plossac)];
con = [con (optimal.Pdc_ic1 == -optimal.Pac_ic1 - optimal.P_quad_ic(1)) ];

% con = [con (optimal.Pdc_ic1 <= state.P(22) + Delta_P)&(optimal.Pdc_ic1 >= state.P(22) - Delta_P)]; 
con = [con (optimal.Pdc_ic3 <= state.P(24) + Delta_P)&(optimal.Pdc_ic3 >= state.P(24) - Delta_P)]; 
...
cost.P_afe = norm([optimal.Pac_ic1, optimal.Pac_ic2 ,optimal.Pac_ic3],2);
cost.P_afe_norm = (cost.P_afe - P_min)/(P_max - P_min);
...
cost.P_afe1 = norm(optimal.Pdc_ic1,2);
cost.P_afe1_norm = (cost.P_afe1 - P_min)/(P_max - P_min);

if     SM_info.mode == "Grid connected"
    cost.sum =     0*cost.P_afe1 + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm +   5*cost.P_afe_norm + 0.1*cost.P_samsung_norm + 0*10*cost.Q_afe1_norm; 
elseif SM_info.mode == "Prepare Island" % here
    cost.sum =   100*cost.P_afe1 + 1000*cost.SOC_sc_norm + 0.1*cost.E_dc_norm + 0*cost.loss_norm + 5*cost.Q_afe_norm + 0.5*cost.P_afe23_norm + 0.1*cost.P_samsung_norm +   10*cost.Q_afe1_norm; 
