function [solution,OPF_termination] = make_optimizer_island_V2(step,SM_info,Grid_para,idx,constraints,variables,prev_solution,OPF_termination)
...
cost.E_afe = (norm([optimal.Eabs(23:26) - Edc_nominal],2));

%minimize losses
cost.loss = norm(optimal.Ploss ,2) + norm(optimal.Qloss ,2); 

%prevent curtailement
cost.P_pv =	norm([  (optimal.Perun_P -    variables.PVmax_P_perun     )],2);

% not needed in theory (losses already accounts for them)
cost.Q_afe = norm([optimal.IC1ac_Q,optimal.IC2ac_Q ,optimal.IC3ac_Q,optimal.IC4ac_Q],2);
% cost.PV_Q =	norm(    optimal.PV_Q_perun ); 

cost.P_samsung = norm([optimal.Samsung_P]);

% minimize for island
cost.P_sc = norm( optimal.SC_P ); 
cost.slack.Q = norm( optimal.IC1ac_Q ); 

% minimize for transition to connected
cost.PCC_ang = norm(optimal.Eang(1) - SM_info.Angle);
cost.PCC_mag = norm(optimal.Eabs(1) - SM_info.Magnitude/(Grid_para.V_b/sqrt(3)));

if     SM_info.mode == "Grid connected"
    cost.sum = cost.P_sc + cost.P_samsung + cost.P_pv + cost.E_afe  + cost.loss + cost.Q_afe; 
elseif SM_info.mode == "Prepare Island"
    cost.sum = cost.P_sc + cost.P_pv + cost.E_afe  + cost.loss + cost.Q_afe;
elseif SM_info.mode == "Island"
    cost.sum = cost.P_sc + cost.E_afe  + cost.loss + cost.Q_afe;
elseif SM_info.mode == "Prepare for connection"
    cost.sum = cost.P_sc + cost.E_afe  + cost.loss + cost.Q_afe + cost.PCC_ang + cost.PCC_mag;
end
...
optimal.Iabs = zeros(Grid_para.n_lines,1);
optimal.Eang  = zeros(Grid_para.n_nodes,1);
...
solution.PV_P_perun = double(optimal.Perun_P);
solution.PVmax_P_perun = double(variables.P(9));
solution.SC_P = double(optimal.SC_P);
