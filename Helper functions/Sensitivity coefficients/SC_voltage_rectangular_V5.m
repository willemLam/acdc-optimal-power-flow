function [K, Time] = SC_voltage_rectangular_V5(E,idx,Grid_para,idxCtrl)
...
dV_imag = eye(size(F));  % WHY NO MINUS
...
% A18   p, Q (idx.vscac_vv)
A18 = [dP_symp_real(idx.pqac,idx.vscdc_vv) ;
       dQ_symp_real(idx.pqac,idx.vscdc_vv) ];

%% P - V
% A21   Ereal, Eimag (idx.pqac)
A21 = [dP_real(idx.pvac,idx.pqac) dP_imag(idx.pvac,idx.pqac);
       dV_mag_real(idx.pvac,idx.pqac) dV_mag_imag(idx.pvac,idx.pqac)];
% A22   Q   ,angE (idx.pvac)
...
A25 = [dP_real(idx.pvac,idx.vscac_pq) dP_imag(idx.pvac,idx.vscac_pq);
       dV_mag_real(idx.pvac,idx.vscac_pq) dV_mag_imag(idx.pvac,idx.vscac_pq)];
% A26   Ereal, Eimag (vscac_vq)
A26 = [dP_real(idx.pvac,idx.vscac_vq) dP_imag(idx.pvac,idx.vscac_vq);
       dV_mag_real(idx.pvac,idx.vscac_vq) dV_mag_imag(idx.pvac,idx.vscac_vq)];
% A27   Edc (vscdc_p)
A27 = [zeros(length(idx.pvac),length(idx.vscdc_pq));
       zeros(length(idx.pvac),length(idx.vscdc_pq))];
% A28   p, Q (idx.vscac_vv)
A28 = [dP_symp_real(idx.pvac,idx.vscdc_vv) ;
       dQ_symp_real(idx.pvac,idx.vscdc_vv) ];
...
% A38   p, Q (idx.vscac_vv)
A38 = [dP_real(idx.pdc,idx.vscdc_vv) ];
...
% A48   p, Q (idx.vscac_vv)
A48 = [dP_real(idx.vdc,idx.vscdc_vv) ];
...
% A58   p, Q (idx.vscac_vv)
A58 = [dP_symp_real(idx.vscac_pq,idx.vscdc_vv) ];
...
A63 = [dP_symp_real(index_vsc,idx.pdc).*repmat(t_inv,length(idx.vscdc_vq),1); %!!!! 
...
% A68   p, Q (idx.vscac_vv)
A68 = [dP_symp_real(idx.vscac_vq,idx.vscdc_vv) ;
       dQ_symp_real(idx.vscac_vq,idx.vscdc_vv) ];
...
% A78   p, Q (idx.vscac_vv)
A78 = [dP_real(idx.vscdc_pq,idx.vscdc_vv) ];

%% VSC VV_dc
% A81   Ereal, Eimag (idx.pqac)
A81 = [dP_symp_real(idx.vscac_vv,idx.pqac) dP_symp_imag(idx.vscac_vv,idx.pqac)];
% A82   Q   ,angE (idx.pvac)
A82 = [dP_symp_real(idx.vscac_vv,idx.pvac) dP_symp_imag(idx.vscac_vv,idx.pvac)];
% A83   Edc (idx.pdc)
A83 = [dP_symp_real(idx.vscdc_vv,idx.pdc)];
% A84   Pdc (idx.vdc)
A84 = [zeros(length(idx.vscdc_vv),length(idx.vdc))];
% A85   Ereal, Eimag (vscac_pq)
A85 = [dP_symp_real(idx.vscac_vv,idx.vscac_pq) dP_symp_imag(idx.vscac_vv,idx.vscac_pq)];
% A86   Ereal, Eimag (vscac_vq)
A86 = [dP_sym_real(idx.vscac_vv,idx.vscac_vq) dP_sym_imag(idx.vscac_vv,idx.vscac_vq)];
% A87   Edc (vscdc_p)
A87 = [dP_symp_real(idx.vscdc_vv,idx.vscdc_pq)];
% A88   p, Q (idx.vscac_vv)
A88 = [dP_symp_real(idx.vscdc_vv,idx.vscdc_vv) ];
...
for id_x = 1:length(idxCtrl)
    ...
    % Identify node-type of idxCtrl
     node_type = 1*sum( idxCtrl(id_x) == idx.slack ) + ...
                ...
                10*sum( idxCtrl(id_x) == idx.vscac_vv ) + ...
                11*sum( idxCtrl(id_x) == idx.vscdc_vv );
    for ctrl_var = 1:2
     ...
     switch( node_type )
         case 1 % Slack node
             ...
             elseif (ctrl_var == 2) 
                K{id_x,4}{ctrl_var,1}(tmp_idx,1) = 1; % imag
             end
             
%          case 3 % PV node  % we work now in rectangular coordinates ->
%          not valid anymore
%              if(ctrl_var == 1) % P
...
%              end
         ...
         case 10 % VSCac_vv node
             if (ctrl_var == 1) 
                K{id_x,3}{ctrl_var,1}(tmp_idx,1) =  real(E(tmp_idx))./abs(E(tmp_idx)); % abs(E(tmp_idx)) ./ (sqrt(abs(E(tmp_idx))^2 - imag(E(tmp_idx))^2));   %real(E(tmp_idx))./abs(E(tmp_idx)); % abs
                K{id_x,4}{ctrl_var,1}(tmp_idx,1) = angle(E(tmp_idx)); % angle
             elseif (ctrl_var == 2) 
                K{id_x,4}{ctrl_var,1}(tmp_idx,1) = imag(E(tmp_idx)); % angle
             end
      ...
      switch( node_type )
         case 1 % slack node
             ...
             elseif (ctrl_var == 2) 
                ...
                u8 = [zeros(length(idx.vscac_vv),1)];
             end
          ...
          case 5 % vdc mode
              if (ctrl_var == 1) 
                ...
                u5 = [- dP_sym_real(idx.vscac_pq,tmp_idx);
                      - dQ_sym_real(idx.vscac_pq,tmp_idx)];
                u6 = [- dP_sym_real(idx.vscdc_vq,tmp_idx);
                      - dQ_sym_real(idx.vscdc_vq,tmp_idx)];            
                u7 = [- dP_real(idx.vscdc_pq,tmp_idx)];
                u8 = [- dP_real(idx.vscdc_vv,tmp_idx)];
             elseif (ctrl_var == 2) 
                u1 = [zeros(length(idx.pqac),1);
                      zeros(length(idx.pqac),1)];
                u2 = [zeros(length(idx.pvac),1);
...
%                       - dQ_sym_real(idx.vscac_vq,tmp_idx)];            
%                 u7 = [- dP_real(idx.vscdc_pq,tmp_idx)];
%                 u8 = [- dP_real(idx.vscac_vv,tmp_idx)];
%              elseif (ctrl_var == 2) 
%                 u1 = [- dP_imag(idx.pqac,tmp_idx);
%                       - dQ_imag(idx.pqac,tmp_idx)];
%                 u2 = [- dP_imag(idx.pvac,tmp_idx);
...
%                       - dQ_sym_imag(idx.vscac_vq,tmp_idx)];
%                 u7 = [zeros(length(idx.vscdc_pq),1)];
%                 u8 = [- dP_imag(idx.vscac_vv,tmp_idx)];
%              end
          
          case 10 % vscac_vv node V2
             if (ctrl_var == 1) 
                u1 = [(- dP_real(idx.pqac,tmp_idx)).* real(E(idx.vscac_vv))./abs(E(idx.vscac_vv)) + (- dP_imag(idx.pqac,tmp_idx)).* imag(E(idx.vscac_vv))./abs(E(idx.vscac_vv));
                      (- dQ_real(idx.pqac,tmp_idx)).* real(E(idx.vscac_vv))./abs(E(idx.vscac_vv)) + (- dQ_imag(idx.pqac,tmp_idx)).* imag(E(idx.vscac_vv))./abs(E(idx.vscac_vv))];  
                u2 = [- dP_real(idx.pvac,tmp_idx);
                      - dV_imag(idx.pvac,tmp_idx)];
                u3 = [- dP_real(idx.pdc,tmp_idx)];
                u4 = [- dVdc(idx.vdc,tmp_idx)];
                ...
                u6 = [- dV_sym_real(idx.vscac_vq,tmp_idx);
                      - dQ_sym_real(idx.vscac_vq,tmp_idx)];            
                u7 = [- dP_real(idx.vscdc_pq,tmp_idx)];
                u8 = [(- dP_real(idx.vscac_vv,tmp_idx)).* real(E(idx.vscac_vv))./abs(E(idx.vscac_vv)) + (- dP_imag(idx.vscac_vv,tmp_idx)).* imag(E(idx.vscac_vv))./abs(E(idx.vscac_vv))];
             elseif (ctrl_var == 2) 
                u1 = [(- dP_real(idx.pqac,tmp_idx)).* imag(E(idx.vscac_vv)) + (- dP_imag(idx.pqac,tmp_idx)).* real(E(idx.vscac_vv));
                      (- dQ_real(idx.pqac,tmp_idx)).* imag(E(idx.vscac_vv)) + (- dQ_imag(idx.pqac,tmp_idx)).* real(E(idx.vscac_vv))];  
                u2 = [- dP_imag(idx.pvac,tmp_idx);
                      - dV_imag(idx.pvac,tmp_idx)];
                u3 = [zeros(length(idx.pdc),1)];
                u4 = [zeros(length(idx.vdc),1)];
                ...
                u6 = [- dV_sym_real(idx.vscac_vq,tmp_idx);
                      - dQ_sym_imag(idx.vscac_vq,tmp_idx)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
                u8 = [(- dP_real(idx.vscac_vv,tmp_idx)).* imag(E(idx.vscac_vv)) + (- dP_imag(idx.vscac_vv,tmp_idx)).* real(E(idx.vscac_vv))];
             end

          case 11 % vscdc_vv node
              if (ctrl_var == 1) 
                u1 = [zeros(length(idx.pqac),1);
                      zeros(length(idx.pqac),1)];
                u2 = [zeros(length(idx.pvac),1);
                ...
                u6 = [zeros(length(idx.vscac_vq),1);
                      zeros(length(idx.vscac_vq),1)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
                u8 = [zeros(length(idx.vscdc_vv),1)];
             elseif (ctrl_var == 2) 
                u1 = [zeros(length(idx.pqac),1);                      
                      zeros(length(idx.pqac),1)];
                u2 = [zeros(length(idx.pvac),1);
     ...
     K{id_x,3}{ctrl_var,1}(idx.vscac_pq,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 1: 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + length(idx.vscac_pq) );
     K{id_x,3}{ctrl_var,1}(idx.vscac_vq,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc)  + 2*length(idx.vscac_pq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc)  + 2*length(idx.vscac_pq) + length(idx.vscac_vq) );
     K{id_x,3}{ctrl_var,1}(idx.vscdc_pq,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc)  + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) );
     % K{id_x,3}{ctrl_var,1}(idx.vscdc_vq,1) =  %Already good (not taken into account)
     % K{id_x,3}{ctrl_var,1}(idx.vscac_vv,1) =  %Already good (not taken into account)
     K{id_x,3}{ctrl_var,1}(idx.vscdc_vv,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc)  + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + length(idx.vscdc_pq) + length(idx.vscdc_vv));
     ...
     K{id_x,4}{ctrl_var,1}(idx.vscac_pq,1) =  x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + length(idx.vscac_pq) + 1: 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) );
     K{id_x,4}{ctrl_var,1}(idx.vscac_vq,1) =  x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) + length(idx.vscac_vq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) );
     % K{id_x,4}{ctrl_var,1}(idx.vscdc_pq,1) =  %DC quantity, imag part is zero
     % K{id_x,4}{ctrl_var,1}(idx.vscdc_vq,1) =  %DC quantity, imag part is zero
     % K{id_x,3}{ctrl_var,1}(idx.vscac_vv,1) =  %Already good (not taken into account)
     % K{id_x,3}{ctrl_var,1}(idx.vscdc_vv,1) =  %DC quantity, imag part is zero
...
% x_rec_P22 = [0.0310314461933740;0.0310314447532749;0.0311370153734342;0.0310211723907850;0.0310211726929037;0.115485910119477;0.139618685930671;0.00520649338627359;0.00275138823242562;-0.0412892607019315;-0.0653790039616852;-0.0464927216122150;-0.0509528301596755;0.140213527454399;0.000299290725447336;-0.0519520764320102;-0.0673850094829665;0.144848649039194;0.00788691461026262;0.00788691646073955;0.00788832016251517;0.00788276832733138;0.00788276764056212;0.0173425770530267;0.0200454066416999;0.00209607946339204;0.00104660199766703;-0.00358996806045276;-0.00497344883130165;-0.00421081798225746;-0.00474297950985606;0.0197932458859174;-2.76011269362376e-06;-0.00519368339459208;-0.00587076813863762;0.0224710787604954;-0.00133319081627965;-0.00533304740270387;-0.0186680806207252;-0.0520750803776887;-0.0676316050052312;0.145495449830171;-0.00519493052729449;-0.00587250337329952;0.0224844626061600;0.00157824305085700]
% x_rec_P21 = [-0.0775662257154780;-0.0775662221158054;-0.0778301065155581;-0.0775405453109747;-0.0775405460661508;-0.288668665825748;-0.348990969982872;-0.0130141546955060;-0.00687737204825490;0.103206666401358;0.163421406361268;0.116213241117650;0.127361731721408;-0.350477836292837;-0.000748107317330063;0.129859448438259;0.168435619236833;-0.362063789613363;-0.0197141375573066;-0.0197141421827601;-0.0197176508767502;-0.0197037735054438;-0.0197037717887948;-0.0433495183498852;-0.0501055131765222;-0.00523936176747576;-0.00261608712270217;0.00897348680281276;0.0124316363545923;0.0105253637237287;0.0118555550692941;-0.0494752119660103;6.89917971787892e-06;0.0129821348308826;0.0146745763548086;-0.0561687249877205;0.00333244474437128;0.0133304892080022;0.0466627480420523;0.130166909191295;0.169052009585552;-0.363680533345938;0.0129852521647023;0.0146789137486451;-0.0562021792581764;-0.00394497749005226]
    end
end
