function [K, Time] = SC_Voltage_V6_rectangular_balanced(S_star,E_star,idx1,idx3,Grid_para,Filter_para,idxCtrl,unblanced_3ph,filter)
...
A56 = [dP_real(idx.vscac_pq,idx.vscac_vq) dP_imag(idx.vscac_pq,idx.vscac_vq);
       dQ_real(idx.vscac_pq,idx.vscac_vq) dQ_imag(idx.vscac_pq,idx.vscac_vq)];
% A57   Edc (vscdc_p)
...
A61 = [dP_real(idx.vscac_vq,idx.pqac) dP_imag(idx.vscac_vq,idx.pqac);
       dQ_real(idx.vscac_vq,idx.pqac) dQ_imag(idx.vscac_vq,idx.pqac)];
% A62   Q   ,angE (idx.pvac)
A62 = [zeros(length(idx.vscac_vq),length(idx.pvac)) dP_imag(idx.vscac_vq,idx.pvac);
       zeros(length(idx.vscac_vq),length(idx.pvac)) dQ_imag(idx.vscac_vq,idx.pvac)];
% A63   Edc (idx.pdc)

index_vsc = repmat(idx.vscdc_vq,1,n_ph)';
A63 = [dP_real(index_vsc,idx.pdc); %!!!! maybe x and y have to be swapped
       dQ_real(index_vsc,idx.pdc)];
% A64   Pdc (idx.vdc)
A64 = [zeros(length(idx.vscac_vq),length(idx.vdc));
       zeros(length(idx.vscac_vq),length(idx.vdc))];
% A65   magE, angE (vscac_pq)
A65 = [dP_real(idx.vscac_vq,idx.vscac_pq) dP_imag(idx.vscac_vq,idx.vscac_pq);
       dQ_real(idx.vscac_vq,idx.vscac_pq) dQ_imag(idx.vscac_vq,idx.vscac_pq)];
% A66   magE, angE (vscac_vq)
A66 = [dP_real(idx.vscac_vq,idx.vscac_vq) dP_imag(idx.vscac_vq,idx.vscac_vq);
       dQ_real(idx.vscac_vq,idx.vscac_vq) dQ_imag(idx.vscac_vq,idx.vscac_vq)];
% A67   Edc (vscdc_p)
A67 = [dP_real(idx.vscac_vq,idx.vscdc_pq);
       dQ_real(idx.vscac_vq,idx.vscdc_pq)];
      ...
      switch( node_type )
         case 1 % slack node
             if (ctrl_var == 1) 
                ...
                u6 = [- dV_real(idx.vscac_vq,tmp_idx);
                      - dQ_real(idx.vscac_vq,tmp_idx)];
             ...
             elseif (ctrl_var == 2) 
                ...
                u6 = [- dV_imag(idx.vscac_vq,tmp_idx);
                      - dQ_imag(idx.vscac_vq,tmp_idx)];
          ...
          case 9 % vscdc_vq node
              if (ctrl_var == 1) 
                ...
                u5 = [zeros(length(idx.vscac_pq),1);
                      zeros(length(idx.vscac_pq),1)];
                u6 = [-dP_real(index_vsc_v,tmp_idx);
                      zeros(length(idx.vscac_vq),1)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
             elseif (ctrl_var == 2) 
                u1 = [zeros(length(idx.pqac),1);                      
