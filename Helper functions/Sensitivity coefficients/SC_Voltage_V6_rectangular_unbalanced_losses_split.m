function [K, Time] = SC_Voltage_V6_rectangular_unbalanced_losses_split(S_star,E_star_augmented,idx1_augmented,idx3_augmented,Grid_para_augmented,Filter_para,idxCtrl,unblanced_3ph,filter)
...
T_inv = 1/3*   [1 1     1; 
                ...
                1 alp^2 alp];
t_inv = [0;1;0];
TCell =  repmat({T_inv}, 1, Grid_para_augmented.n_ac);
tCell =  repmat({t_inv}, 1, Grid_para_augmented.n_ac);
ICell =  repmat({1}, 1, Grid_para_augmented.n_dc); 
...
% sequence powers
F_sym = conj(Grid_para_augmented.YY).*(Ttot*E_star_augmented);
H_sym = diag(Grid_para_augmented.YY*(Ttot*E_star_augmented));
...
indexes_0 = 1:3:Grid_para_augmented.n_ac*Grid_para_augmented.n_ph;
indexes_n = 3:3:Grid_para_augmented.n_ac*Grid_para_augmented.n_ph;
...
A56a = [dP_symp_real(idx.vscac_pq,idx.vscac_vq_v) dP_symp_imag(idx.vscac_pq,idx.vscac_vq_v);
       dQ_symp_real(idx.vscac_pq,idx.vscac_vq_v) dQ_symp_imag(idx.vscac_pq,idx.vscac_vq_v)];
% A56   magE, angE (vscac_vq)
A56b = [dP_symp_real(idx.vscac_pq,idx.vscac_vq_q) dP_symp_imag(idx.vscac_pq,idx.vscac_vq_q);
       dQ_symp_real(idx.vscac_pq,idx.vscac_vq_q) dQ_symp_imag(idx.vscac_pq,idx.vscac_vq_q)];
% A57   Edc (vscdc_p)
...
A6a1 = [dP_symp_real(idx.vscac_vq_v,idx.pqac) dP_symp_imag(idx.vscac_vq_v,idx.pqac);
        dQ_symp_real(idx.vscac_vq_v,idx.pqac) dQ_symp_imag(idx.vscac_vq_v,idx.pqac)];
% A62   Q   ,angE (idx.pvac)
A6a2 = [zeros(length(idx.vscac_vq_v),length(idx.pvac)) dP_symp_imag(idx.vscac_vq_v,idx.pvac);
       zeros(length(idx.vscac_vq_v),length(idx.pvac)) dQ_symp_imag(idx.vscac_vq_v,idx.pvac)];
% A63   Edc (idx.pdc)

index_vsc_v = repmat(idx.vscdc_vq,1,n_ph)';
A6a3 = [dP_symp_real(index_vsc_v,idx.pdc).*repmat([0;1;0],length(idx.vscdc_vq),1); %!!!! maybe x and y have to be swapped
        dQ_symp_real(idx.vscac_vq_v,idx.pdc).*repmat([0;1;0],length(idx.vscdc_vq),1)]; % ??
% A64   Pdc (idx.vdc)
A6a4 = [zeros(length(idx.vscac_vq_v),length(idx.vdc));
       zeros(length(idx.vscac_vq_v),length(idx.vdc))];
% A65   magE, angE (vscac_pq)
A6a5 = [dP_symp_real(idx.vscac_vq_v,idx.vscac_pq) dP_symp_imag(idx.vscac_vq_v,idx.vscac_pq);
        dQ_symp_real(idx.vscac_vq_v,idx.vscac_pq) dQ_symp_imag(idx.vscac_vq_v,idx.vscac_pq)];
% A66   magE, angE (vscac_vq)
A6a6a = [dP_sym_real(idx.vscac_vq_v,idx.vscac_vq_v) dP_sym_imag(idx.vscac_vq_v,idx.vscac_vq_v);
         dQ_sym_real(idx.vscac_vq_v,idx.vscac_vq_v) dQ_sym_imag(idx.vscac_vq_v,idx.vscac_vq_v)];
% A66   magE, angE (vscac_vq)
A6a6b = [dP_symp_real(idx.vscac_vq_v,idx.vscac_vq_q) dP_symp_imag(idx.vscac_vq_v,idx.vscac_vq_q);
         dQ_symp_real(idx.vscac_vq_v,idx.vscac_vq_q) dQ_symp_imag(idx.vscac_vq_v,idx.vscac_vq_q)];
% A67   Edc (vscdc_p)
A6a7 = [dP_symp_real(idx.vscac_vq_v,idx.vscdc_pq);
        dQ_symp_real(idx.vscac_vq_v,idx.vscdc_pq)];
   
%% VSC VdcQ_Q
% A61   magE, angE (idx.pqac)
A6b1 = [dP_symp_real(idx.vscac_vq_q,idx.pqac) dP_symp_imag(idx.vscac_vq_q,idx.pqac);
        dQ_symp_real(idx.vscac_vq_q,idx.pqac) dQ_symp_imag(idx.vscac_vq_q,idx.pqac)];
% A62   Q   ,angE (idx.pvac)
A6b2 = [zeros(length(idx.vscac_vq_q),length(idx.pvac)) dP_symp_imag(idx.vscac_vq_q,idx.pvac);
        zeros(length(idx.vscac_vq_q),length(idx.pvac)) dQ_symp_imag(idx.vscac_vq_q,idx.pvac)];
% A63   Edc (idx.pdc)
A6b3 = [dP_symp_real(idx.vscac_vq_q,idx.pdc);
        dQ_symp_real(idx.vscac_vq_q,idx.pdc)];
% A64   Pdc (idx.vdc)
A6b4 = [zeros(length(idx.vscac_vq_q),length(idx.vdc));
        zeros(length(idx.vscac_vq_q),length(idx.vdc))];
% A65   magE, angE (vscac_pq)
A6b5 = [dP_symp_real(idx.vscac_vq_q,idx.vscac_pq) dP_symp_imag(idx.vscac_vq_q,idx.vscac_pq);
        dQ_symp_real(idx.vscac_vq_q,idx.vscac_pq) dQ_symp_imag(idx.vscac_vq_q,idx.vscac_pq)];
% A66   magE, angE (vscac_vq)
A6b6a = [dP_symp_real(idx.vscac_vq_q,idx.vscac_vq_v) dP_symp_imag(idx.vscac_vq_q,idx.vscac_vq_v);
         dQ_symp_real(idx.vscac_vq_q,idx.vscac_vq_v) dQ_symp_imag(idx.vscac_vq_q,idx.vscac_vq_v)];
% A66   magE, angE (vscac_vq)
A6b6b = [dP_sym_real(idx.vscac_vq_q,idx.vscac_vq_q) dP_sym_imag(idx.vscac_vq_q,idx.vscac_vq_q);
         dQ_sym_real(idx.vscac_vq_q,idx.vscac_vq_q) dQ_sym_imag(idx.vscac_vq_q,idx.vscac_vq_q)];
% A67   Edc (vscdc_p)
A6b7 = [dP_symp_real(idx.vscac_vq_q,idx.vscdc_pq);
        dQ_symp_real(idx.vscac_vq_q,idx.vscdc_pq)];
      ...
      switch( node_type )
         case 1 % slack node
             if (ctrl_var == 1) 
                ...
                u6a = [- dV_sym_real(idx.vscac_vq_v,tmp_idx);
                      - dQ_sym_real(idx.vscac_vq_v,tmp_idx)];
                u6b = [- dV_sym_real(idx.vscac_vq_q,tmp_idx);
                      - dQ_sym_real(idx.vscac_vq_q,tmp_idx)];             
             ...
             elseif (ctrl_var == 2) 
                ...
                u6a = [- dV_sym_real(idx.vscac_vq_v,tmp_idx);
                      - dQ_sym_imag(idx.vscac_vq_v,tmp_idx)];
                u6b = [- dV_sym_real(idx.vscac_vq_q,tmp_idx);
                      - dQ_sym_imag(idx.vscac_vq_q,tmp_idx)];
           ...
           case 7 % vscac_vq_v node
             ...
             elseif (ctrl_var == 2) 
                ...
                u5 = [zeros(length(idx.vscac_pq),1);
                      zeros(length(idx.vscac_pq),1)]; 
                u6a = [zeros(length(idx.vscac_vq_v),1);
                      -dV_sym_imag(idx.vscac_vq_v,tmp_idx)];
                u6b = [zeros(length(idx.vscac_vq_q),1);
                      zeros(length(idx.vscac_vq_q),1)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
              end
             ...
             elseif (ctrl_var == 2) 
                ...
                u6a = [zeros(length(idx.vscac_vq_v),1);
                      zeros(length(idx.vscac_vq_v),1)];
                u6b = [zeros(length(idx.vscac_vq_q),1);
                      -dV_sym_imag(idx.vscac_vq_q,tmp_idx)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
              end
          ...
          case 10 % vscdc_vq node
              if (ctrl_var == 1) 
                ...
                u5 = [zeros(length(idx.vscac_pq),1);
                      zeros(length(idx.vscac_pq),1)];
                u6a = [-dP_sym_real(index_vsc_v,tmp_idx).*repmat([0;1;0],length(idx.vscdc_vq),1);
                      zeros(length(idx.vscac_vq_v),1)];
                u6b = [zeros(length(idx.vscac_vq_q),1);
                      zeros(length(idx.vscac_vq_q),1)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
