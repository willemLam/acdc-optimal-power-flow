function [K, Time] = SC_voltage_rectangular_V4(E,idx,Grid_para,Filter_para,idxCtrl)
...
% losses
% b*abs(Iac) + c* Iac^2 + a +  d* Idc + e* Idc^2
% b = 0;

G = zeros(Grid_para.n_nodes);

if Filter_para.Include_losses == 1
    for m = 1:length(Grid_para.pos_dc3(:,1))
        ...
        k = Grid_para.pos_dc3(m,1);
        j = Grid_para.pos_dc3(m,2);
        l = Grid_para.pos_ac3(m,1);
        i = Grid_para.pos_ac3(m,2);
    
        Ykk = real(Grid_para.YY(k,k));
        Ykj = real(Grid_para.YY(k,j));
        Yll = Grid_para.YY(l,l);
        Yli = Grid_para.YY(l,i);
        ...
        alpha = (real(Yll)*real(El) - imag(Yll)*imag(El) + real(Yli)*real(Ei) - imag(Yli)*imag(El))^2;
        beta  = (imag(Yll)*real(El) + real(Yll)*imag(El) + imag(Yli)*real(Ei) + real(Yli)*imag(El))^2;
        
        dP_dEk = a + d*Ykk + 2*e*Ykk^2*Ek + 2*e*Ykk*Ykj*Ej;
        dP_dEj =     d*Ykj + 2*e*Ykj^2*Ej + 2*e*Ykk*Ykj*Ek;
        
        dP_dEl_re =  2*c*alpha*real(Yll) + 2*c*beta*imag(Yll);
        dP_dEl_im = -2*c*alpha*imag(Yll) + 2*c*beta*real(Yll);
        
        dP_dEi_re =  2*c*alpha*real(Yli) + 2*c*beta*imag(Yli);
        dP_dEi_im = -2*c*alpha*imag(Yli) + 2*c*beta*real(Yli);
        
        %DC part
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,1)) = dP_dEk;
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,2)) = dP_dEj;
        %AC part
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_ac3(m,1)) = complex(dP_dEl_re,dP_dEl_im );
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_ac3(m,2)) = complex(dP_dEi_re,dP_dEi_im );
        
    %     Yloss_dc = Grid_para.YY(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,2));
    %     %DC part
    %     G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,1)) = Filter_para.a(m) + Filter_para.d(m) .*Yloss_dc + 2*Filter_para.e(m) .* (sign(Grid_para.YY(Grid_para.pos_dc3(m,1),:)).*((Grid_para.YY(Grid_para.pos_dc3(m,1),:)).^2)*E);
    %     G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,2)) =                  - Filter_para.d(m) .*Yloss_dc - 2*Filter_para.e(m) .* (sign(Grid_para.YY(Grid_para.pos_dc3(m,1),:)).*((Grid_para.YY(Grid_para.pos_dc3(m,1),:)).^2)*E);
     
    
    end
end

dP_real =  real(F) + real(H) + real(G);
dP_imag =  imag(F) + imag(H) + imag(G);
...
dP_sym_real =  real(F_sym) + real(H_sym) + real(G_sym);
dP_sym_imag =  imag(F_sym) + imag(H_sym) + imag(G_sym);
dQ_sym_real = -imag(F_sym) + imag(H_sym) ;
dQ_sym_imag =  real(F_sym) - real(H_sym) ;
...
A58 = [dP_symp_real(idx.vscac_pq,idx.vscdc_vv);
       dQ_symp_real(idx.vscac_pq,idx.vscdc_vv)];
...
% A67   Edc (vscdc_p)
%A67 = [dP_symp_real(index_vsc,idx.vscdc_pq);
...
%% Assemble A
A = [ A11 A12 A13 A15 A16 A17 A18; ...
      A21 A22 A23 A25 A26 A27 A28; ...
      A31 A32 A33 A35 A36 A37 A38; ...
      A51 A52 A53 A55 A56 A57 A58; ...
      A61 A62 A63 A65 A66 A67 A68; ...
      A71 A72 A73 A75 A76 A77 A78; ...
...
%              end
         ...
         case 10 % VSCac_vv node
             ...
             elseif (ctrl_var == 2) 
                K{id_x,4}{ctrl_var,1}(tmp_idx,1) = real(E(tmp_idx)); %imag(E(tmp_idx)); % angle
             end
...
%              end
     ...
     K{id_x,3}{ctrl_var,1}(idx.vscdc_vv,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc)  + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + length(idx.vscdc_vv));
