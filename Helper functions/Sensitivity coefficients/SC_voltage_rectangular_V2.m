function [K, Time] = SC_voltage_rectangular_V2(E,idx,Grid_para,idxCtrl)
...
A81 = [dP_symp_real(idx.vscdc_vv,idx.pqac) dP_symp_imag(idx.vscdc_vv,idx.pqac)];
% A82   Q   ,angE (idx.pvac)
A82 = [dP_symp_real(idx.vscdc_vv,idx.pvac) dP_symp_imag(idx.vscdc_vv,idx.pvac)];
% A83   Edc (idx.pdc)
A83 = [dP_symp_real(idx.vscdc_vv,idx.pdc)];
% A84   Pdc (idx.vdc)
A84 = [zeros(length(idx.vscdc_vv),length(idx.vdc))];
% A85   Ereal, Eimag (vscac_pq)
A85 = [dP_symp_real(idx.vscdc_vv,idx.vscac_pq) dP_symp_imag(idx.vscdc_vv,idx.vscac_pq)];
% A86   Ereal, Eimag (vscac_vq)
A86 = [dP_sym_real(idx.vscdc_vv,idx.vscac_vq) dP_sym_imag(idx.vscdc_vv,idx.vscac_vq)];
% A87   Edc (vscdc_p)
...
%              end
          ...
          case 11 % vscdc_vv node
              if (ctrl_var == 1) 
                ...
                u6 = [zeros(length(idx.vscac_vq),1);
                      zeros(length(idx.vscac_vq),1)];
                u7 = [zeros(length(idx.vscdc_pq),1)];
                u8 = [-dP_sym_real(idx.vscdc_vv,tmp_idx)];
             elseif (ctrl_var == 2) 
                u1 = [zeros(length(idx.pqac),1);                      
                      zeros(length(idx.pqac),1)];
                u2 = [zeros(length(idx.pvac),1);
     ...
     K{id_x,3}{ctrl_var,1}(idx.vscdc_vv,1) =   x( 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + length(idx.vdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + 1 : 2*length(idx.pqac) + 2*length(idx.pvac) + length(idx.pdc) + length(idx.vdc) + 2*length(idx.vscac_pq) + 2*length(idx.vscac_vq) + length(idx.vscdc_pq) + length(idx.vscdc_pq) + length(idx.vscdc_vv));
