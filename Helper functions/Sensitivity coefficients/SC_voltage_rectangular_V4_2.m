function [K, Time] = SC_voltage_rectangular_V4_2(E,idx,Grid_para,Filter_para,idxCtrl)
...
if Filter_para.Include_losses == 1
    for m = 1:length(Grid_para.pos_dc3(:,1))
        ...
        dEk = a + d*Ykk + 2*e*Ykk^2*Ej + 2*e*Ykk*Ykj*Ej;
        dEj =     d*Ykj + 2*e*Ykj^2*Ej + 2*e*Ykk*Ykj*Ek;
        
        dEl_re =  2*c*alpha*real(Yll) + 2*c*beta*imag(Yll);
        dEl_im = -2*c*alpha*imag(Yll) + 2*c*beta*real(Yll);
        
        dEi_re =  2*c*alpha*real(Yli) + 2*c*beta*imag(Yli);
        dEi_im = -2*c*alpha*imag(Yli) + 2*c*beta*real(Yli);
        
        %DC part
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,1)) = dEk;
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_dc3(m,2)) = dEj;
        %AC part
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_ac3(m,1)) = complex(dEl_re,dEl_im );
        G(Grid_para.pos_dc3(m,1),Grid_para.pos_ac3(m,2)) = complex(dEi_re,dEi_im );
          ...
          case 10 % vscac_vv node V2
             if (ctrl_var == 1) 
                u1 = [- dP_real(idx.pqac,tmp_idx);
                      - dQ_real(idx.pqac,tmp_idx)];  
                u2 = [- dP_real(idx.pvac,tmp_idx);
                      - dQ_real(idx.pvac,tmp_idx)];
                u3 = [- dP_real(idx.pdc,tmp_idx)];
                u4 = [- dVdc(idx.vdc,tmp_idx)];
                u5 = [- dP_sym_real(idx.vscac_pq,tmp_idx);
                      - dQ_sym_real(idx.vscac_pq,tmp_idx)];
                u6 = [- dP_sym_real(idx.vscac_vq,tmp_idx);
                      - dQ_sym_real(idx.vscac_vq,tmp_idx)];            
                u7 = [- dP_real(idx.vscdc_pq,tmp_idx)];
                u8 = [- dP_real(idx.vscac_vv,tmp_idx)];
...
%                 u8 = [(- dP_real(idx.vscac_vv,tmp_idx)).* real(E(idx.vscac_vv))./abs(E(idx.vscac_vv)) + (- dP_imag(idx.vscac_vv,tmp_idx)).* imag(E(idx.vscac_vv))./abs(E(idx.vscac_vv))];
             elseif (ctrl_var == 2) 
                u1 = [- dP_imag(idx.pqac,tmp_idx);
                      - dQ_imag(idx.pqac,tmp_idx)];  
                u2 = [- dP_imag(idx.pvac,tmp_idx);
                      - dQ_imag(idx.pvac,tmp_idx)];
                u3 = [- dP_imag(idx.pdc,tmp_idx)];
                u4 = [- dV_imag(idx.vdc,tmp_idx)];
                u5 = [- dP_sym_imag(idx.vscac_pq,tmp_idx);
                      - dQ_sym_imag(idx.vscac_pq,tmp_idx)];
                u6 = [- dV_sym_imag(idx.vscac_vq,tmp_idx);
                      - dQ_sym_imag(idx.vscac_vq,tmp_idx)];
                u7 = [- dP_imag(idx.vscdc_pq,tmp_idx)];
                u8 = [- dP_imag(idx.vscac_vv,tmp_idx)];
             end
