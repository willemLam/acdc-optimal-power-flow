function variables = Receive_data_from_CDA(Grid_para,variables)
...
[message, ~] = judp('RECEIVE',CDA_port,2^15,4000); 
json1 = char(message);
data.SCADA.Data = loadjson(json1');
...
%% PV -> from MeteoBox
    ...
    variables.PVmax_P_perun = max(0,real(S(9)));  %Perun is off
    variables.PVmax_P_solarmax = max(0,0.7*real(S(11))); %previous PV (accounting for the curtailment)
    variables.PVmax_P_facade = max(0,0.3*real(S(11))); %previous PV 
    variables.PVmax_P_emul4 = 0.01;
    variables.PVmax_P_emul5 = 0.01;
    variables.PVmax_P_emul6 = 0.01;
    variables.PVmax_P_emul7 = 0.01;
