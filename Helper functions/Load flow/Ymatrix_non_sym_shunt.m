function [YY, YYL, YL, YT, YYT, Ib, Ampacities, y_ih, y_i, A, linedata]  = Ymatrix_non_sym_shunt(Text,A_b,V_b,ZIN)
...
%from  to   R_long  X_long      B_from      G_from      B_to        G_to         ?  Amp length
...
omega = 2*pi*50;
if size(linedata,2) == 9
%     line_lengths = linedata(:,6);
    line_lengths = linedata(:,9);
    linedata(:,3)=(linedata(:,3).*line_lengths.*Yb); %
    linedata(:,4)=(omega*linedata(:,4).*line_lengths.*Yb);
    linedata(:,5)=(omega*linedata(:,5).*line_lengths./Yb);
    linedata(:,6)=(      linedata(:,5).*line_lengths./Yb)*0;
    linedata(:,7)=(omega*linedata(:,5).*line_lengths./Yb);
    linedata(:,8)=(      linedata(:,5).*line_lengths./Yb)*0;
else
%     line_lengths = linedata(:,9);
    line_lengths = linedata(:,11);
    linedata(:,3)=(linedata(:,3).*line_lengths.*Yb); %R %.*line_lengths
    linedata(:,4)=(omega*linedata(:,4).*line_lengths.*Yb); %X
    linedata(:,5)=(omega*linedata(:,5).*line_lengths./Yb); %B_from
    linedata(:,6)=(      linedata(:,6).*line_lengths./Yb); %G_from
    linedata(:,7)=(omega*linedata(:,7).*line_lengths./Yb); %B_to
    linedata(:,8)=(      linedata(:,8).*line_lengths./Yb); %G_to
end
%----------------------------------------------------------------------------------------------%
...
for k=1:n_lines
    y_i_ih(linedata(k,1),linedata(k,2))=linedata(k,6) + linedata(k,5)*1i;
    y_i_ih(linedata(k,2),linedata(k,1))=linedata(k,8) + linedata(k,7)*1i;
end
...
YY = Y;
YYL = -(YY - diag(diag(YY)));
YYT = - y_i_ih; %-(A.'*YT_cur*A - diag(diag(A.'*YT_cur*A)));
Ampacities = (-(A.'*diag(linedata(:,10))*A - diag(diag(A.'*diag(linedata(:,10))*A))))/Ib;
