function [E,J,n_iter,time] = NR_rectangularACDC_1ph_general_V2_quadratic_loss(Grid_para,Filter_para,S_star,E_star,E_0,idx,tol,n_max)
% INPUT
% - Grid_para   
% - Filter_para
...
t = tic ;
% Initialization
E_0([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]) = E_star([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]);
E_re = real(E_0);
E_im = imag(E_0);
dx = 1;
...
for k=1:n_max
    ...
    dF = Mismatch_1ph_V2_quadratic_loss(E,S,E_star,S_star,Grid_para,Filter_para,idx);
...
%     [J_PpR, J_PpX, J_QpR, J_QpX] = Jacobian_Powers_symmetric(E_re, E_im, Grid_para, J_PpR, J_PpX, J_QpR, J_QpX);
    ...
    [J_AFEiR, J_AFEiX, J_AFErR, J_AFErX] = Jacobian_Converters_1ph_V2_quadratic_loss(E_re,E_im, E_star, Grid_para, Filter_para, J_EdiorR, J_EdiorX, J_EdioiR, J_EdioiX, J_PR, J_PX, J_QR, J_QX, J_AFEiR, J_AFEiX, J_AFErR, J_AFErX);
...
%     J_AFEiX(sort([idx.slack;idx.pqac;idx.pvac;idx.pdc;idx.vdc;idx.vscdc_pq;idx.vscdc_vq]),:)=[];%ac
    ...
    J_PR(:,sort([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]))=[];
    J_PX(:,sort([idx.slack;idx.vdc;idx.pdc;idx.vscdc_pq;idx.vscdc_vq;idx.vscac_vv;idx.vscdc_vv]))=[];
    
    J_QR(:,sort([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]))=[];
    J_QX(:,sort([idx.slack;idx.vdc;idx.pdc;idx.vscdc_pq;idx.vscdc_vq;idx.vscac_vv;idx.vscdc_vv]))=[];

    J_ER(:,sort([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]))=[];
    J_EX(:,sort([idx.slack;idx.vdc;idx.pdc;idx.vscdc_pq;idx.vscdc_vq;idx.vscac_vv;idx.vscdc_vv]))=[];
    
    J_AFErR(:,sort([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]))=[];
    J_AFErX(:,sort([idx.slack;idx.vdc;idx.pdc;idx.vscdc_pq;idx.vscdc_vq;idx.vscac_vv;idx.vscdc_vv]))=[];
    J_AFEiR(:,sort([idx.slack;idx.vdc;idx.vscdc_vq;idx.vscac_vv]))=[];
    J_AFEiX(:,sort([idx.slack;idx.vdc;idx.pdc;idx.vscdc_pq;idx.vscdc_vq;idx.vscac_vv;idx.vscdc_vv]))=[];
     ...
     J =[J_PR(idx.pqac,:),J_PX(idx.pqac,:); %PQac real
         ...
         J_AFErR(idx.vscac_vv,:),J_AFErX(idx.vscac_vv,:); % VSC DC Vdc-Q
%          J_AFEiR(idx.vscac_vv,:),J_AFEiX(idx.vscac_vv,:); % VSC AC Vdc-Q
         J_PR(idx.vscdc_pq,:),J_PX(idx.vscdc_pq,:); %Pdc
         J_PR(idx.pdc,:),J_PX(idx.pdc,:)];
         %J_ER(idx.vscdc_vq,:),J_EX(idx.vscdc_vq,:)]; %Vdc
     
     %J_ER(idx.vdc,:),J_EX(idx.vdc,:)
    %% Solution update
    ...
    dE_re(sort([idx.pqac;idx.pvac;idx.pdc;idx.vscac_pq;idx.vscac_vq;idx.vscdc_pq;idx.vscdc_vv]),1)...
        = dx(1:length(idx.pqac) + length(idx.pvac) + length(idx.pdc) +...
                 length(idx.vscac_pq) + length(idx.vscac_vq)  + length(idx.vscdc_pq) + length(idx.vscdc_vv) );
    ...
    dE_im(sort([idx.pqac;idx.pvac;idx.vscac_vq;idx.vscac_pq]),1)...
        = dx( length(idx.pqac) + length(idx.pvac) + length(idx.pdc) +...
                 length(idx.vscac_pq) + length(idx.vscac_vq) + length(idx.vscdc_pq) + length(idx.vscdc_vv) +  1: end);
